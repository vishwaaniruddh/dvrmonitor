<?php
// hik2.php - Async batch dashboard for Hikvision, inspired by cpplus/index.php batch_ui
require_once __DIR__ . '/../vendor/autoload.php';
require_once __DIR__ . '/HikvisionApiClient.php';
require_once __DIR__ . '/hik_camera_api.php';
require_once __DIR__ . '/hik_hard_disk_api.php';
require_once __DIR__ . '/hik_time_api.php';
...
// --- Config ---
$endpoints = [
    'camera' => '/ISAPI/System/Video/inputs/channels',
    'hdd' => '/ISAPI/ContentMgmt/Storage',
...
if (isset($_GET['batch_ui'])) {
    ?>
    <h2 style="text-align:center;">Hikvision DVR Dashboard</h2>
            ...
            .then(dvrs => {
                ...
                function processBatch() {
                    ...
                    const promises = batch.map(dvr => {
                        ...
                        return fetch(`hik2.php?api_status=1&ip=${encodeURIComponent(dvr.ip)}&port=${dvr.port}&username=${encodeURIComponent(dvr.username)}&password=${encodeURIComponent(dvr.password)}`)
                            .then(r => r.json())
                            .then(res => {
...
if (isset($_GET['get_dvrs'])) {
    ...
    $result = $mysqli->query("SELECT IPAddress as ip, port, UserName as username, Password as password FROM all_dvr_live WHERE dvrname = 'Hikvision' AND LOWER(live)='y'  LIMIT 50");
    $dvrs = [];
